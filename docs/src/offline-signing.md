...
This document describes using the CLI to separately sign and submit a
transaction.
...
```bash
solana transfer --sign-only --blockhash 5Tx8F3jgSHx21CbtjwmdaKPLM5tWmreWAnPrbqHomSJF recipient-keypair.json 1
```
...
Typically a transaction must be signed and accepted by the network within
