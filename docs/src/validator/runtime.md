...
- `Transfer` - Transfers sparks (equivalent to lamports) between accounts.
...
- Runtime guarantees that the program can only spend sparks (equivalent to lamports) that are in accounts that are assigned to it.
