...
smart contracts elsewhere) to the Powerledger blockchain with the Solana CLI tools.
...
To deploy a program, use the CLI tools to interact with the on-chain loader
to:
...
`max-len` is fixed after initial deployment, so any POWR in the program accounts
...
Or to recover the account's sparks, pass it as the
...
### Closing program and buffer accounts, and reclaiming their sparks
...
sparks. The buffer account can either be used to [resume a
