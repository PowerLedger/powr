---
title: Using CLI with Powerledger Blockchain
---

Before running any CLI commands, let's go over some conventions that
you will see across all commands. 

First, the CLI tools are provided by the Solana Foundation, so will use the `solana` binaries for the time being. The CLI tools are actually a collection
